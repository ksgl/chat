...
public:
    Implementation(DateTimeDecorator* dateTimeDecorator, const QDateTime& value)
        ...
        , value(value)
        , locale(QLocale(QLocale::English, QLocale::AnyCountry))
    {
...
QString DateTimeDecorator::toPrettyString() const
{
    if (implementation->value.isNull()) {
        ...
        QDateTime date = implementation->value;
        return implementation->locale.toString(date, "ddd d MMM yyyy @ HH:mm:ss" );
    }
}
...
QString DateTimeDecorator::toPrettyDateString() const
{
    if (implementation->value.isNull()) {
        ...
        QDateTime date = implementation->value;
        return implementation->locale.toString(date, "d MMM yyyy" );
    }
}
...
QString DateTimeDecorator::toPrettyTimeString() const
{
    if (implementation->value.isNull()) {
        ...
        QDateTime date = implementation->value;
        return implementation->locale.toString(date, "hh:mm ap" );
    }
}
